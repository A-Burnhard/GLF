...
function show_sidebar_menu(with_animation) {
    ...
    if (!with_animation) {
        ...
        public_vars.$sidebarMenu.add($sidebar_inner).transit({
        ...
        setTimeout(function() {
            ...
            TweenMax.to($logo, (public_vars.sidebarTransitionTime / 2) / 1100, {
                css: {
                    scaleY: 1
                }
            });
            setTimeout(function() {
                ...
                setTimeout(function() {
                    public_vars.$pageContainer.add(public_vars.$sidebarMenu).add($sidebar_inner).add($logo_env).add($logo).add($span_elements).add($submenus).attr('style', '');
...
function hide_sidebar_menu(with_animation) {
    ...
    if (!with_animation) {
        ...
        $logo.transit({
            ...
            $logo.hide();
            public_vars.$sidebarMenu.transit({
        ...
        TweenMax.to($submenus, public_vars.sidebarTransitionTime / 1100, {
            css: {
                height: 0
            }
        });
        ...
        setTimeout(function() {
            public_vars.$pageContainer.add(public_vars.$sidebarMenu).add($sidebar_inner).add($search_input).add($search_button).add($user_link).add($logo_env).add($logo).add($span_elements).add($collapse_icon).add($submenus).add($sidebar_ulink).add(public_vars.$mainMenu).attr('style', '');
            public_vars.$pageContainer.addClass(public_vars.sidebarCollapseClass);
            public_vars.$mainMenu.data('is-busy', false);
